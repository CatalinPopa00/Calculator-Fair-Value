...
def calculate_scoring_reform(valuation_data, metrics):
    ...
    hybrid_peg = clean_ratio(metrics.get('peg_custom'))
    if hybrid_peg <= 0:
...
def get_scoring_rules(valuation_data: dict, metrics: dict) -> dict:
    ...
    # BUY RULES
    mos_criteria = ["Depends on company's moat (ROIC > 20% & Health > 70/100)", "Strong Moat: > 15% => Max, > 0% => High, > -15% => Half", "No Moat: > 15% => Max, > 5% => High, > -5% => Half"]
    growth_mega = ["Mega Cap: >= 15% => Max, 10-15% => High, 5-10% => Half"]
    ...
    if is_high_growth:
        b_rules = [
            ...
            {"name": "EV/Gross Profit (1Y Fwd)", "criteria": ["<= Target => 25p", "<= Target*1.3 => 12.5p", "Higher => 0p"]},
        ...
        if is_fintech:
            ...
            b_rules = [
                ...
                {"name": "P/E Ratio (1Y Fwd)", "criteria": ["<= Target => 20p", "<= Target*1.3 => 10p", "Higher => 0p", "Moat/Monopoly: Tolerates up to 10% premium"]},
