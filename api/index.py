...
def deep_clean_data(val):
    ...
    if isinstance(val, list):
        return [deep_clean_data(v) for v in val]
    if hasattr(val, "item"): # Handle numpy scalars fallback
        try:
            return val.item()
        except Exception:
            pass
...
def get_valuation(ticker: str, response: Response, wacc: float = None, fast_mode: bool = False, skip_peers: bool = False, force_refresh: bool = False):
    ...
    try:
        ...
        if price_fx != 1.0:
            ...
            for key in ["adjusted_eps", "trailing_eps", "fwd_eps", "forward_eps", "fcf", "total_cash", "total_debt", "revenue", "forward_revenue", "ebitda", "eps_last_year", "open", "regularMarketOpen", "previousClose", "previous_close"]:
