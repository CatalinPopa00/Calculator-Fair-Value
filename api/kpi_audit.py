...
'''
    ...
    try:
        ...
        for kpi in result.get('kpis', []):
            kpi_name_lower = kpi.get('name', '').lower().strip()
            if kpi_name_lower in banned_exact or any(banned in kpi_name_lower for banned in banned_contains):
                print(f"[Audit] Discarding banned generic metric generated by AI: {kpi.get('name')}")
                continue
            filtered_kpis.append(kpi)
            
        result['kpis'] = filtered_kpis
        # --------------------------------------------------------

        # Salvare în cache
        audit_cache[ticker] = result
        kv_set(redis_key, json.dumps(result), ex=2592000) # 30 days
        return result
