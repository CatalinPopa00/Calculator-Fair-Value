...
const animatePriceUI = (openPrice, newPrice, triggerFlash = true) => {
    ...
    const displayData = (data, isSilentUpdate = false) => {
        ...
        if (fvContainer) {
            ...
            const renderCorporateBrief = (synthesisText, isLoadingAI = false) => {
                ...
                if (pe != null && pe > 0) {
                    if (pe > 45) {
                        kpiHtml += `<span style="background: rgba(239, 68, 68, 0.12); color: #f87171; border: 1px solid rgba(239, 68, 68, 0.2); font-size: 0.65rem; padding: 3px 8px; border-radius: 12px; font-weight: 700; letter-spacing: 0.3px; display: inline-flex; align-items: center; justify-content: center; gap: 4px; box-sizing: border-box;">🔴 Premium PE (${pe.toFixed(1)}x)</span>`;
                    } else if (pe < 18) {
                        kpiHtml += `<span style="background: rgba(34, 197, 94, 0.12); color: #4ade80; border: 1px solid rgba(34, 197, 94, 0.2); font-size: 0.65rem; padding: 3px 8px; border-radius: 12px; font-weight: 700; letter-spacing: 0.3px; display: inline-flex; align-items: center; justify-content: center; gap: 4px; box-sizing: border-box;">🟢 Attractive PE (${pe.toFixed(1)}x)</span>`;
                    } else {
                        kpiHtml += `<span style="background: rgba(245, 158, 11, 0.12); color: #fbbf24; border: 1px solid rgba(245, 158, 11, 0.2); font-size: 0.65rem; padding: 3px 8px; border-radius: 12px; font-weight: 700; letter-spacing: 0.3px; display: inline-flex; align-items: center; justify-content: center; gap: 4px; box-sizing: border-box;">🟡 Moderate PE (${pe.toFixed(1)}x)</span>`;
                    }
                }
                if (netMargin != null) {
                    if (netMargin > 1.0) {
                        kpiHtml += `<span style="background: rgba(245, 158, 11, 0.12); color: #fbbf24; border: 1px solid rgba(245, 158, 11, 0.2); font-size: 0.65rem; padding: 3px 8px; border-radius: 12px; font-weight: 700; letter-spacing: 0.3px; display: inline-flex; align-items: center; justify-content: center; gap: 4px; box-sizing: border-box;" title="One-off exceptional gain exceeding 100% of revenue.">⚠️ Exceptional Profit (${(netMargin * 100).toFixed(0)}%)</span>`;
                    } else if (netMargin > 0.20) {
                        kpiHtml += `<span style="background: rgba(56, 189, 248, 0.12); color: #38bdf8; border: 1px solid rgba(56, 189, 248, 0.2); font-size: 0.65rem; padding: 3px 8px; border-radius: 12px; font-weight: 700; letter-spacing: 0.3px; display: inline-flex; align-items: center; justify-content: center; gap: 4px; box-sizing: border-box;">💎 High Margins (${(netMargin * 100).toFixed(0)}%)</span>`;
                    } else if (netMargin < 0.05) {
                        kpiHtml += `<span style="background: rgba(239, 68, 68, 0.12); color: #f87171; border: 1px solid rgba(239, 68, 68, 0.2); font-size: 0.65rem; padding: 3px 8px; border-radius: 12px; font-weight: 700; letter-spacing: 0.3px; display: inline-flex; align-items: center; justify-content: center; gap: 4px; box-sizing: border-box;">⚠️ Low Margins (${(netMargin * 100).toFixed(0)}%)</span>`;
                    } else {
                        kpiHtml += `<span style="background: rgba(255, 255, 255, 0.05); color: rgba(255,255,255,0.7); border: 1px solid rgba(255, 255, 255, 0.1); font-size: 0.65rem; padding: 3px 8px; border-radius: 12px; font-weight: 700; letter-spacing: 0.3px; display: inline-flex; align-items: center; justify-content: center; gap: 4px; box-sizing: border-box;">📊 Healthy Margins (${(netMargin * 100).toFixed(0)}%)</span>`;
                    }
                }
                if (deRatio != null) {
                    if (deRatio < 40) {
                        kpiHtml += `<span style="background: rgba(168, 85, 247, 0.12); color: #c084fc; border: 1px solid rgba(168, 85, 247, 0.2); font-size: 0.65rem; padding: 3px 8px; border-radius: 12px; font-weight: 700; letter-spacing: 0.3px; display: inline-flex; align-items: center; justify-content: center; gap: 4px; box-sizing: border-box;">🛡️ Safe Debt Level</span>`;
                    } else if (deRatio > 150) {
                        kpiHtml += `<span style="background: rgba(239, 68, 68, 0.12); color: #f87171; border: 1px solid rgba(239, 68, 68, 0.2); font-size: 0.65rem; padding: 3px 8px; border-radius: 12px; font-weight: 700; letter-spacing: 0.3px; display: inline-flex; align-items: center; justify-content: center; gap: 4px; box-sizing: border-box;">⚠️ High Debt Level (${deRatio.toFixed(0)}%)</span>`;
                    } else {
                        kpiHtml += `<span style="background: rgba(255, 255, 255, 0.05); color: rgba(255,255,255,0.7); border: 1px solid rgba(255, 255, 255, 0.1); font-size: 0.65rem; padding: 3px 8px; border-radius: 12px; font-weight: 700; letter-spacing: 0.3px; display: inline-flex; align-items: center; justify-content: center; gap: 4px; box-sizing: border-box;">⚖️ Balanced Debt</span>`;
                    }
                }
                ...
                if (!isLoadingAI) {
                    ...
                    if (hasPharma) {
                        kpiHtml += `<span class="insight-badge" style="background: rgba(16, 185, 129, 0.12); color: #34d399; border: 1px solid rgba(16, 185, 129, 0.2); font-size: 0.65rem; padding: 3px 8px; border-radius: 12px; font-weight: 700; letter-spacing: 0.3px; display: inline-flex; align-items: center; justify-content: center; gap: 4px; box-sizing: border-box; animation: brief-pulse 2s infinite;" title="Clinical pipeline, FDA decision, or testing phase detected.">🧪 Clinical Catalyst</span>`;
                    }
                    if (hasMa) {
                        kpiHtml += `<span class="insight-badge" style="background: rgba(245, 158, 11, 0.12); color: #fbbf24; border: 1px solid rgba(245, 158, 11, 0.2); font-size: 0.65rem; padding: 3px 8px; border-radius: 12px; font-weight: 700; letter-spacing: 0.3px; display: inline-flex; align-items: center; justify-content: center; gap: 4px; box-sizing: border-box;" title="M&A activity, mergers, or integration costs detected.">🤝 M&A Transaction</span>`;
                    }
                    if (hasSegment) {
                        kpiHtml += `<span class="insight-badge" style="background: rgba(56, 189, 248, 0.12); color: #38bdf8; border: 1px solid rgba(56, 189, 248, 0.2); font-size: 0.65rem; padding: 3px 8px; border-radius: 12px; font-weight: 700; letter-spacing: 0.3px; display: inline-flex; align-items: center; justify-content: center; gap: 4px; box-sizing: border-box;" title="Specific analysis on business segments or divisions.">📈 Segment Focus</span>`;
                    }
                }
        ...
        try {
            ...
            window._renderEstimatesTable = () => {
                ...
                eItems.forEach((item, idx) => {
                    ...
                    let gVal = item.growth != null ? (parseFloat(item.growth) * 100).toFixed(1) + '%' : '--';
    ...
    csInputs.forEach(input => {
        const showTooltip = (e) => {
            ...
            const getAnchor = (idx, key) => {
                if (anchors.length > idx && anchors[idx]) return anchors[idx][key];
                return null;
            };
            ...
            const formatVal = (v) => v !== 'N/A' && !isNaN(v) && v !== null ? v.toFixed(1) + (title.includes('P/E') || title.includes('MULTIPLE') ? 'x' : '%') : 'N/A';

            if (id.includes('rev-1-3')) {
                title = "REV GROWTH TIPS";
                labelFwd = "FWD:";
                const rev0 = getAnchor(0, 'revenue_b');
                const rev1 = getAnchor(1, 'revenue_b');
                const rev3 = getAnchor(3, 'revenue_b');
                ...
                if (rev0 > 0) {
                     ...
                     else if (estRev.growth != null) valFwd = estRev.growth * 100;
                     else if (estRev.avg != null) valFwd = ((estRev.avg / rev0_abs) - 1) * 100;
                }
                ...
                const rev0 = getAnchor(0, 'revenue_b');
                const fcf0 = getAnchor(0, 'fcf_b');
                if (rev0 > 0 && fcf0 != null) val1y = (fcf0 / rev0) * 100;

                let avgMargin = 0;
                let count = 0;
                for (let i = 0; i < 3; i++) {
                    const r = getAnchor(i, 'revenue_b');
                    const f = getAnchor(i, 'fcf_b');
                    if (r > 0 && f != null) {
                        avgMargin += (f / r) * 100;
                        count++;
                    }
                ...
                labelFwd = "FWD:";
                const eps0 = getAnchor(0, 'eps');
                const eps1 = getAnchor(1, 'eps');
                const eps3 = getAnchor(3, 'eps');
                ...
                if (eEsts.length >= 2) {
                    ...
                    const estLast = eEsts.length > 2 ? eEsts[2] : eEsts[1];
                    const yearsDiff = eEsts.length > 2 ? 2 : 1;
                    labelFwd = `FWD ${yearsDiff}y:`;
                ...
                if (valFwd === 'N/A') {
                     if (prof.eps_growth != null) fwdBase = prof.eps_growth * 100;
                     else fwdBase = val1y;
                     valFwd = getFwdScenario(fwdBase);
                }
                
                labelFwd5y = "FWD 5Y:";
                if (globalData.computed_eps_growth != null) {
                     valFwd5y = globalData.computed_eps_growth * 100;
                } else if (prof.earnings_growth != null) {
                     valFwd5y = prof.earnings_growth * 100;
                }
                ...
                const currentEps = prof.adjusted_eps || prof.trailing_eps || getAnchor(0, 'eps');
            ...
            if (valFwd5y !== null) {
                htmlContent += `<div style="display:flex; justify-content:space-between; width:130px; margin-top:3px;"><span style="color:var(--text-muted);">${labelFwd5y}</span> <span style="color:#fbbf24;">${formatVal(valFwd5y)}</span></div>`;
            }
    ...
    document.body.addEventListener('click', (e) => {
        ...
        if (model === 'dcf' && currentFormulaData.dcf) {
            ...
            if (peers.length > 0) {
                peerTableHTML = `
                    <h4 style="font-size:0.95rem; color:var(--text-muted); text-transform:uppercase; letter-spacing:1px; margin-bottom:10px;">Peer Benchmarks</h4>
                    <div style="overflow-x:auto; margin-bottom:1.5rem;">
                    <table class="premium-data-table" style="font-size:0.85rem;">
                        <thead>
                            <tr style="border-bottom:1px solid rgba(255,255,255,0.15);">
                                <th style="text-align:left; padding:6px 2px; color:white; font-size:0.85rem; white-space:nowrap;">Ticker</th>
                                ${activeKeys.map(k => `<th style="text-align:right; padding:6px 2px; color:white; font-size:0.85rem; white-space:nowrap;">${LABEL[k] || k}</th>`).join('')}
                            </tr>
                        </thead>
                        <tbody>
                            <!-- Target Company Row -->
                            <tr style="border-bottom:1px solid rgba(255,255,255,0.1); background: rgba(40, 199, 111, 0.05);">
                                <td style="padding:6px 2px; color:#28c76f; font-weight:700; white-space:nowrap;">${(globalData.ticker || 'TARGET').toUpperCase()}</td>
                                ${activeKeys.map(k => {
                    let val = null;
                    ...
                    return `<td style="text-align:right; padding:6px 2px; color:#28c76f; font-weight:700; white-space:nowrap;">${val != null ? val.toFixed(1) + 'x' : '—'}</td>`;
                }).join('')}
                            </tr>
                            ...
                            ${peers.map(p => `
                                <tr style="border-bottom:1px solid rgba(255,255,255,0.04);">
                                    <td style="padding:6px 2px; color:white; font-weight:600; white-space:nowrap;">${p.ticker}</td>
                                    ${activeKeys.map(k => {
                    const dk = peerKeyMap[k];
                    const val = dk ? p[dk] : null;
                    return `<td style="text-align:right; padding:6px 2px; color:var(--text-main); white-space:nowrap;">${val != null && val !== 0 ? val.toFixed(1) + 'x' : '—'}</td>`;
                }).join('')}
                                </tr>
                            `).join('')}
                        </tbody>
                        <tfoot>
                            <tr style="border-top:1px solid rgba(255,255,255,0.15);">
                                <td style="padding:6px 2px; color:white; font-weight:700; white-space:nowrap;">Median</td>
                                ${activeKeys.map(k => {
                    let medKey = k;
                    if (k === 'P_FFO') medKey = 'PE';
                    if (k === 'P_AFFO') medKey = 'PFCF';
                    const v = dynamicMedians[medKey] ?? r['median_peer_' + medKey.toLowerCase()];
                    return `<td style="text-align:right; padding:6px 2px; color:white; font-weight:700; white-space:nowrap;">${v != null ? v.toFixed(1) + 'x' : '—'}</td>`;
                }).join('')}
                            </tr>
                        </tfoot>
                    </table>
            ...
            html = `
                    ...
                    <h4 style="font-size:0.95rem; color:var(--text-muted); text-transform:uppercase; letter-spacing:1px; margin-bottom:10px;">Implied Values & Weights</h4>
                    ...
                    <div style="display:flex; justify-content:space-between; align-items:center; padding:12px 0; border-top:1px solid rgba(255,255,255,0.1);">
                        <span style="font-size:0.95rem; color:white; font-weight:600;">Weighted Fair Value</span>
                        <span id="rel-modal-fv" style="font-size:1.3rem; font-weight:800; color:${modalFVColor};">$${fmt(modalFV)}</span>
                    </div>
                    <p style="font-size:0.8rem; color:var(--text-muted); margin-top:8px; font-style:italic;">
